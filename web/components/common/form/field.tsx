...
export const BaseField: React.FC<FieldProps> = (props) => {
  ...
  return (
    <FormControl variant={variant} {...controlProps} isInvalid={!!error}>
      {label && !hideLabel ? <FormLabel variant={variant}>{label}</FormLabel> : null}
      <Box>
        {children}
    ...
    return (
      <Controller
        ...
        render={({ field: { ref: _ref, ...field } }) => (
          <InputComponent {...field} {...inputProps} ref={useMergeRefs(ref, _ref)} />
        )}
      />
    );
  });
...
export const withUncontrolledInput = (InputComponent: React.FC<any>) => {
  return forwardRef<FieldProps, typeof InputComponent>(({ name, rules, ...inputProps }, ref) => {
    ...
    return <InputComponent {...field} {...inputProps} ref={useMergeRefs(ref, _ref)} />;
  });
};
...
export const RadioField = registerFieldType<RadioProps>('radio', Radio, {
  isControlled: true,
});
